...
async def test_is_broken_returns_true_if_all_attempts_fail(mocker):
    ...
    async with respx.mock:
        # All 5 attempts fail (we increased max_attempts to 5)
        ...
        assert head_route.call_count == 5
...
async def test_is_broken_handles_429_with_longer_sleep(mocker):
    ...
    async with respx.mock:
        # First attempt: HEAD 429 -> fallback to GET 429 -> sleep base_delay * 2^0 = 5
        ...
        # Should have slept 5s after first 429 (base_delay * 2^0)
